          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 flex flex-col gap-3">
            ...
            {userNotifications.length > 0 ? (
              <div className="bg-purple-50 p-3 rounded-lg border border-purple-100">
                <p className="text-xs text-purple-900 font-medium line-clamp-3">
                  {userNotifications[0].message}
                </p>
                <p className="text-[10px] text-purple-500 mt-1 text-right">
                  Mới nhất
                </p>
              </div>
            ) : (
                  ...
                  paginatedHistory.map((record) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <button
                          className="p-2 text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-full transition-colors"
                          title="Xem chi tiết"
                        >
        ...
        {totalHistoryPages > 1 && (
          <div className="grid grid-cols-1 sm:grid-cols-3 items-center gap-4 border-t border-slate-100 pt-4">
            <p className="text-xs text-slate-500 text-center sm:text-left">
              Trang{" "}
              <span className="font-medium text-slate-800">{historyPage}</span>{" "}
              / {totalHistoryPages}
            </p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                ...
                <span>•</span>
                <span>{patient.age ? `${patient.age} tuổi` : "N/A"}</span>
              </div>
            </div>
          </div>
          <button
              ...
              historyRecords.map((record, index) => (
                ...
                >
                  {index !== historyRecords.length - 1 && (
                    <div className="absolute left-[3px] top-3 bottom-0 w-0.5 bg-slate-200 group-last:hidden"></div>
                  )}
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="bg-blue-50/50 rounded-lg p-3 border border-blue-100">
                      <span className="text-xs font-bold text-blue-600 uppercase tracking-wide block mb-1 flex items-center gap-1.5">
                        <StickyNote className="w-3 h-3" /> Toa thuốc đã kê
                      </span>
...
const AppointmentCard: React.FC<{
  ...
  const cardStyles = isOnline
    ? "bg-gradient-to-r from-cyan-50 to-white border-cyan-100 hover:border-cyan-200"
    : "bg-gradient-to-r from-amber-50 to-white border-amber-100 hover:border-amber-200";
  ...
  return (
    <div
      className={`p-4 rounded-xl border transition-all duration-200 ${cardStyles}`}
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            />
            <div
              className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full flex items-center justify-center border-2 border-white ${iconBg}`}
            >
              {isOnline ? (
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-sm text-slate-500">
              ...
              {isOnline &&
                !isPending &&
                timeRemaining &&
                timeRemaining !== "Đang diễn ra" && (
                  <div className="flex items-center gap-1.5 text-cyan-600 bg-cyan-50 px-2 py-0.5 rounded-md border border-cyan-100">
          ...
          {isOnline &&
            (appointment.status === "Đã xác nhận" ||
              ...
              <button
                ...
                className="w-full sm:w-auto px-6 py-2.5 bg-cyan-600 text-white font-bold rounded-lg hover:bg-cyan-700 text-sm transition-colors shadow-md shadow-cyan-100"
              >
                Bắt đầu khám
              </button>
            )}
          ...
          {!isOnline &&
            (appointment.status === "Đã xác nhận" ||
              ...
              <div className="flex gap-2">
                <div className="w-full sm:w-auto flex items-center justify-center sm:justify-end gap-2 px-4 py-2.5 bg-slate-50 text-slate-700 font-bold rounded-lg border border-slate-200 text-sm">
                  <Clock className="w-4 h-4" />
                  <span>Khám lúc {formatTimeDisplay(appointment.time)}</span>
                </div>
                <button
                  onClick={() => onRequestCancel(appointment.id)}
                  className="flex items-center justify-center gap-2 bg-red-50 text-red-700 font-semibold py-2.5 px-3 rounded-lg border border-red-100 hover:bg-red-100 hover:border-red-200 text-sm transition-all"
                >
          ...
          {isPending && (
            <>
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-emerald-50 text-emerald-700 font-semibold py-2.5 px-4 rounded-lg border border-emerald-100 hover:bg-emerald-100 hover:border-emerald-200 text-sm transition-all"
              >
                <ShieldCheck className="w-4 h-4" /> <span>Xác nhận</span>
              </button>
              <button
                onClick={() => onRequestCancel(appointment.id)}
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-red-50 text-red-700 font-semibold py-2.5 px-4 rounded-lg border border-red-100 hover:bg-red-100 hover:border-red-200 text-sm transition-all"
              >
                <XCircle className="w-4 h-4" /> <span>Hủy</span>
              </button>
            </>
          )}
        </div>
...
const StatCard: React.FC<{
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-5 hover:shadow-md transition-shadow">
      <div className={`${theme.iconBg} p-3.5 rounded-xl`}>
