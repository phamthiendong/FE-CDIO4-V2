...
export const DoctorScheduleManager: React.FC<DoctorScheduleManagerProps> = ({ initialSlots, onGoBack }) => {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6 p-4 sm:p-6 lg:p-8">
            ...
            {/* PHẦN ĐÃ CHỈNH SỬA: Box chọn ngày nhỏ gọn hơn */}
            <div className="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-slate-200/80">
                <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <span className="font-semibold text-slate-700 text-sm w-28 text-center">{monthYearLabel}</span>
                        <button 
                    ...
                    {weekDays.map(day => {
                        ...
                        return (
                            <button
                                ...
                                disabled={isPast}
                                // Class đã được điều chỉnh: py-2, text nhỏ hơn, rounded-lg
                                className={`flex-shrink-0 text-center py-2 rounded-lg border transition-all duration-200 ${
                                    isSelected ? 'bg-cyan-600 text-white border-cyan-600 shadow-sm' : 'bg-slate-50 hover:bg-slate-100 border-transparent'
                                } ${isPast ? 'opacity-40 cursor-not-allowed bg-slate-50 text-slate-400' : 'cursor-pointer'}`}
                            >
                                <p className={`text-[10px] uppercase font-medium mb-0.5 ${isSelected ? 'text-cyan-100' : 'text-slate-500'}`}>
                                    {day.toLocaleDateString('vi-VN', { weekday: 'short' })}
                                </p>
                                <p className="font-bold text-lg leading-none mb-0.5">{day.getDate()}</p>
                                {/* Ẩn bớt tháng ở view nhỏ, hoặc hiển thị rất nhỏ */}
                                <p className={`text-[9px] ${isSelected ? 'text-cyan-100' : 'text-slate-400'}`}>
...
const SlotCard: React.FC<{ slot: TimeSlot; onDelete: (id: string) => void }> = ({ slot, onDelete }) => {
    ...
    // LOGIC: Offline = màu vàng (amber), Online = màu xanh (green)
    const containerClasses = slot.type === 'offline' 
        ? 'bg-amber-50 border-amber-200' 
        : 'bg-green-50 border-green-200';
    
    const progressBarColor = slot.type === 'offline'
        ? 'bg-amber-500'
        : 'bg-green-500';

    return (
        <div className={`p-4 rounded-xl border ${containerClasses} flex flex-col justify-between`}>
           <div>
