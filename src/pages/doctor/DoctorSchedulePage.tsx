...
// --- CẬP NHẬT 1: Interface nhận thêm reason ---
interface DoctorSchedulePageProps {
  ...
  // 👇 Cập nhật dòng này để nhận reason
  onCancelAppointment: (appointmentId: string, reason: string) => void;
}

type DoctorTab = "schedule" | "reporting";

// --- CẬP NHẬT 2: Component CancelModal (Mới) ---
interface CancelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (reason: string) => void;
  isPending: boolean; // Để đổi tiêu đề Hủy hoặc Từ chối
}
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm text-slate-600">
            {isPending
              ...
              : "Bạn có chắc chắn muốn hủy lịch hẹn đã xác nhận này?"}
            <br />
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Nhập lý do (ví dụ: Bận đột xuất, Sai thông tin...)"
              value={reason}
              onChange={(e) => {
...
export const DoctorSchedulePage: React.FC<DoctorSchedulePageProps> = (
  props
) => {
...
// --- Schedule Tab ---
const ScheduleTab: React.FC<DoctorSchedulePageProps> = ({
  appointments,
  users,
  onStartConsultation,
  ...
  // --- CẬP NHẬT 3: State quản lý Modal ---
  const [isCancelModalOpen, setIsCancelModalOpen] = useState(false);
  const [selectedAptId, setSelectedAptId] = useState<string | null>(null);
  const [isSelectedAptPending, setIsSelectedAptPending] = useState(false);

  const pendingAppointments = appointments.filter(
    (apt) => apt.status === "Chờ xác nhận"
  );
  const confirmedAppointments = appointments.filter(
    (apt) => apt.status === "Đã xác nhận" || apt.status === "Sắp diễn ra"
  );
  const completedAppointments = appointments.filter(
    (apt) => apt.status === "Đã hoàn thành"
  );

  // Hàm mở modal
  const handleRequestCancel = (aptId: string, isPending: boolean) => {
    setSelectedAptId(aptId);
    setIsSelectedAptPending(isPending);
    setIsCancelModalOpen(true);
  };

  // Hàm xử lý khi bấm xác nhận trong modal
  const handleConfirmCancelModal = (reason: string) => {
    if (selectedAptId) {
      onCancelAppointment(selectedAptId, reason);
    }
  };
        ...
        <div className="space-y-4">
          {apts.map((apt) => (
            <AppointmentCard
              ...
              // 👇 Thay vì gọi trực tiếp, ta gọi hàm mở modal
              onRequestCancel={(id) => handleRequestCancel(id, isPending)}
              isPending={isPending}
            />
          ))}
        </div>
  ...
  return (
    <div className="space-y-8">
      {/* Modal component được đặt ở đây */}
      <CancelModal
        isOpen={isCancelModalOpen}
        onClose={() => setIsCancelModalOpen(false)}
        onConfirm={handleConfirmCancelModal}
        isPending={isSelectedAptPending}
      />
...
const ReportingTab: React.FC<{
  ...
  const revenueData = useMemo(() => {
    ...
    const dailyRevenueData = Array.from({ length: 7 })
      .map((_, i) => {
        const date = new Date();
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <StatCard
          ...
          value={`${revenueData.thisMonthRevenue.toLocaleString("vi-VN")}đ`}
          icon={CurrencyDollarIcon}
        />
      </div>
      <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-200">
        <BarChart
          data={revenueData.dailyRevenueData}
...
interface AppointmentCardProps {
  ...
  onViewPatientHistory: (patientId: string) => void;
  onConfirmAppointment: (appointmentId: string) => void;
  // 👇 Sửa props để nhận handler mở modal
  onRequestCancel: (appointmentId: string) => void;
  isPending?: boolean;
}
      ...
      <div className="flex items-center gap-3 w-full sm:w-auto">
        ...
        {(appointment.status === "Đã xác nhận" ||
          ...
          <>
            ...
            {/* Nút Hủy cho lịch đã xác nhận */}
            <button
              onClick={() => onRequestCancel(appointment.id)}
              className="flex-1 sm:flex-none flex items-center gap-2 bg-red-100 text-red-700 font-bold py-2 px-4 rounded-lg hover:bg-red-200 text-sm"
            >
        ...
        {isPending && (
          <>
            <button
              ...
              className="flex-1 sm:flex-none flex items-center gap-2 bg-green-100 text-green-700 font-bold py-2 px-4 rounded-lg hover:bg-green-200 text-sm"
            >
